        ...
        for player in match_info['players']:
            if player['account_id'] == dota_id:
                await botTextured.say(content='```Hero:        {}\nK/D/A:       {}/{}/{}\nLast hits:   {}\nGPM:         {}\nXPM:         {}```'
                                      .format(player['hero_name'], player['kills'], player['deaths'], player['assists'], 
...
async def mymmr(ctx):
    ...
    search_url = base_url + str(usersDotaID[discord_id])
    with urllib.request.urlopen(search_url) as url:
        data = json.loads(url.read().decode())

    await botTextured.say(content='```steam name:    {}\nsolo mmr:      {}\nparty mmr:     {}\nestimated mmr: {}\n```'
                          .format(match['profile']['personaname'], match['solo_competitive_rank'],
